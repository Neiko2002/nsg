...
void IndexNSG::sync_prune(
    unsigned q, std::vector<Neighbor>& pool, const Parameters& parameter, boost::dynamic_bitset<>& flags, SimpleNeighbor* cut_graph_) {
    ...
    while (result.size() < range && (++start) < pool.size() && start < maxc) {
        ...
        for (unsigned t = 0; t < result.size(); t++) {
            ...
            float djk =
                distance_->compare(data_ + dimension_ * (size_t)result[t].id, data_ + dimension_ * (size_t)p.id, (unsigned)dimension_);
...
void IndexNSG::InterInsert(unsigned n, unsigned range, std::vector<std::mutex>& locks, SimpleNeighbor* cut_graph_) {
            ...
            while (result.size() < range && (++start) < temp_pool.size()) {
                ...
                for (unsigned t = 0; t < result.size(); t++) {
                    ...
                    float djk = distance_->compare(
                        data_ + dimension_ * (size_t)result[t].id, data_ + dimension_ * (size_t)p.id, (unsigned)dimension_);
                    if (djk < p.distance /* dik */) {
                        occlude = true;
                        break;
                    }
...
#pragma omp for schedule(dynamic, 100)
        for (int64_t n = 0; n < nd_; ++n) {
            ...
            if (cnt % step_size == 0) {
                ...
                std::cout << progress++ << "/" << percent << " completed. memory usage: " << getCurrentRSS() / 1000000
                          << " Mb, peak memory usage: " << getPeakRSS() / 1000000 << std::endl;
            }
        }
    }
...
void IndexNSG::Explore(const unsigned initial_node_id,
    ...
    auto L = 0;
    {
        ...
        for (unsigned m = 0; m < MaxM; ++m) {
            ...
            if (distance_computation_count >= max_distance_computation_count) break;
        }
    }
    std::sort(retset.begin(), retset.begin() + L);
    ...
    for (; L < K_target; L++) retset[L] = Neighbor(max_id, max_dist, false);
    ...
    for (size_t i = 0; i < L; i++) indices[i] = retset[i].id;
}
...
void IndexNSG::SearchWithOptGraph(const float* query, size_t K, const Parameters& parameters, unsigned* indices) {
    ...
    while (k < (int)L) {
        ...
        if (retset[k].flag) {
            ...
            neighbors++;
            for (unsigned m = 0; m < MaxM; ++m) _mm_prefetch(opt_graph_ + node_size * neighbors[m], _MM_HINT_T0);
            for (unsigned m = 0; m < MaxM; ++m) {
